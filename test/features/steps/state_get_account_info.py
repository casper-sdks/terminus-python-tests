...
def state_get_account_invoked(ctx):
    ...
    ctx.state_account_info = ctx.sdk_client_rpc.get_account_info(ctx.sender_key.account_key.hex(),
                                                                 ctx.sdk_client_rpc.get_block())
...
def a_valid_account(ctx):
    ...
    assert ctx.user_account['account_hash'] == ctx.state_account_info['account_hash']
...
def a_valid_purse_uref(ctx):
    ...
    assert ctx.user_account['main_purse'] == ctx.state_account_info['main_purse']
...
def valid_action_thresholds(ctx):
    ...
    assert ctx.user_account['action_thresholds'] == \
