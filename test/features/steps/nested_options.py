...
def nested_option_created(ctx, _type, _value):
    ...
    ctx.cl_option = CLV_Option(CLV_Option(__val, _utils.cl_types_map['CLV_Option']),
                                    _utils.cl_types_map['CLV_' + _type])
...
def nested_option_has_values(ctx, _type_inner, _value):
    ...
    assert isinstance(ctx.cl_option, CLV_Option)
    assert isinstance(ctx.cl_option.value, _utils.cl_values_map[_type_inner])
    assert ctx.cl_option.value == _value
...
def bytes_are(ctx, hex_bytes):
    ...
    assert to_bytes(ctx.cl_option).hex() == hex_bytes
...
def option_list_has_values(ctx, _type, _val1, _val2, _val3):
    ...
    AttributeError: type object 'CLV_Type_List' has no attribute 'inner_type'    
    The SDK is not checking for nested complex cl_types in the CLV_Option_type 
    """
    ...
    ctx.cl_option = CLV_Option(CLV_List(_list), CLT_Type_List)

    assert ctx.cl_option
    assert isinstance(ctx.cl_option.value, CLV_List)
...
def option_tuple_is_created(ctx, _type1, _type2, _val1, _val2):
    ...
    """
    This test passes, but fails to deploy
    AttributeError: type object 'CLV_Type_Tuple2' has no attribute 't0_type'
    The SDK is not checking for nested complex cl_types in the CLV_Option_type    
    """
    ...
    _tuple = CLV_Tuple2(__val1, __val2)

    ctx.cl_option = CLV_Option(_tuple, _utils.cl_types_map['CLV_Tuple2'])
...
def option_map_has_values(ctx, _type1, _type2, _val1, _val2):
    ...
    """
    This test passes, but fails to deploy
    AttributeError: type object 'CLV_Type_Map' has no attribute 'key_type'
    The SDK is not checking for nested complex cl_types in the CLV_Option_type 
    """

    _list = []
    _tuple: typing.Tuple = (CLV_String(_val1), CLV_U32(int(_val2)))
    _list.append(_tuple)

    _map = CLV_Map(_list)

    ctx.cl_option = CLV_Option(_map, _utils.cl_types_map['CLV_Map'])
...
def option_map_created(ctx, _type1, _type2, _val1, _val2):
    ...
    assert isinstance(ctx.cl_option, CLV_Option)
    assert isinstance(ctx.cl_option.value, CLV_Map)
    assert isinstance(ctx.cl_option.value.value[0][0], CLV_String)
    assert isinstance(ctx.cl_option.value.value[0][1], CLV_U32)
    assert ctx.cl_option.value.value[0][0] == _val1
    assert ctx.cl_option.value.value[0][1] == int(_val2)
...
def option_any_created(ctx, _value):
    ...
    CLV_Any encoding 'NotImplemented' in the SDK
    """

    ctx.cl_option = CLV_Option(CLV_Any(CLV_String(_value)), CLV_Any)
...
def inner_type_is_any(ctx, _value):
    ...
    assert isinstance(ctx.cl_option, CLV_Option)
    assert isinstance(ctx.cl_option.value, CLV_Any)
    assert isinstance(ctx.cl_option.value.value, CLV_String)
